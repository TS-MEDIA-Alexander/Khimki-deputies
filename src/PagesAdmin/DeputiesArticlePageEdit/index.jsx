...
import s from './DeputiesArticlePageEdit.module.css';
...
const DeputiesArticlePageEdit = ({ level }) => {
   ...
   const deputId = useParams().id.slice(3);
   ...
   const schema = yup.object({
      ...
      image_preview: yup.mixed().nullable(),
      image_preview_url: yup.string().nullable(),
   }).test(
      'eitherImagePreviewOrImagePreviewUrl',
      'Нужно выбрать файл или указать URL изображения',
      function (values) {
         const { image_preview, image_preview_url } = values;
         return !!image_preview || !!image_preview_url;
      }
   );
   ...
   const loadDeputies = useCallback(async () => {
      setIsReloading(true);
      try {
         const dataParties = await API.getParty();
         handler(dataParties, 'parties');

         const data = await API.getItemDeputat(deputId)
         const formattedData = {
            ...getValues(),
   ...
   useEffect(() => {
      if (deputId) { // Загружаем данные только если newsId есть
         loadDeputies();
      } else {
         setIsReloading(false);
      }
   ...
   return (
      <div className="mt54">
         <ContantContainerAdmin>
            ...
            ) : <><h1 className="pageTitle mt40">Редактировать депутата</h1>
               <form onSubmit={handleSubmit(onSubmit)} className="text text_admin mt40">
                  ...
                  <div className={`inputContainer mt24`}>
                     ...
                     <ReactSelect
                        ...
                        placeholder={'Партия'}
                        initialValue={getValues('party')}//Выбранное/ые данные по дефолту. Если isMulti true, то передавать массив!
...
export default DeputiesArticlePageEdit;